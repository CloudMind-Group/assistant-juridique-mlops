...
| E-13 | `POST /documents/analyze` reçoit `document_name: str` sans modèle Pydantic : FastAPI en fait un **paramètre de requête**, quel que soit le verbe HTTP. Or le collecteur ne supprime aucun attribut d'URL, et l'instrumentation HTTP renseigne `url.full` par défaut. Un nom de document — potentiellement un nom de fichier source, donc de partie — serait stocké dans la trace, et au-delà dans les journaux d'accès, le proxy et l'historique du navigateur, que nul collecteur n'expurge. Même famille qu'E-R13 : un identifiant de fichier qui fuit par un canal qu'on n'avait pas regardé. Deux correctifs complémentaires : supprimer les attributs d'URL au collecteur (M7), et porter `document_name` dans un modèle de corps de requête (M5). *Correction du 13/09/2026 : la première version de cette ligne citait `/chat` avec la question dans l'URL. Vérifié sur la branche de la PR #54 : `/chat` et `/chat/stream` reçoivent la question dans le corps JSON (`ChatRequest`). J'avais repris l'exemple de l'issue #78 sans le confronter au code ; @youssefelalem l'a fait en relecture et a trouvé le point réel à la ligne suivante.* | Moyenne | M5 + M7 | avant instrumentation de M5 |
