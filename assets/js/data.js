...
const MEMBERS = [
  {
    ...
    subs:[
      ...
      ['Versioning DVC avec remote distant DAGsHub — les données transitent hors de Git, partagées via dvc push/pull',1],
      ...
      ['Chunking sémantique, embeddings et intégration Great Expectations en CI',0]
    ],
    tools:['Python','Pydantic','Apache Airflow','DVC','DAGsHub','Tesseract OCR (à intégrer)','Great Expectations (à intégrer)'],
    ...
    deliverables:['DAG Airflow <code>legal_ingest_v2</code> (structure prête, exécution quotidienne à valider)','Pipeline <code>dvc.yaml</code> versionné, données partagées via le remote DAGsHub','Rapport de qualité de données généré à chaque exécution']
  },
  {
